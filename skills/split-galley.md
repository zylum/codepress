...
2. Read `05-domain/index.md` for module boundaries
...
5. Create `02-product/{galley-id}/run-sheet.md` with ordered Slug list
6. Create each Slug at `02-product/{galley-id}/slugs/{id}-{name}.md`
7. For each Slug, set:
   - Purpose (one sentence)
   - Acceptance checklist (3–7 items)
   - Autonomy level (`continuous` / `confirm` / `stop`)
   - Dependencies (if any)
8. Create `02-product/{galley-id}/review.md` from the template (blank, ready)
9. Update `03-delivery/board.md` — move card from Shaping to Delivering
