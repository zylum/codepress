...
- **Decoupling invariant:** the manifest and any emitted payloads name only CodePress. No reference to any consumer system.
- Stay model-agnostic and dependency-light (bash + markdown). No new runtime services.
- Conform to `loop-spec` v0.1. If the spec needs to change, change it there and bump `spec_version` — don't fork it.
...
- [x] No file in CodePress names any consumer system
